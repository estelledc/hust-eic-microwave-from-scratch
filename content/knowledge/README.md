...
2026-06-03 已按“全量反查”口径复核知识讲义：01-05 阶段逐页补入“逐题反查闭环”，06-08 阶段逐页补入“一致性复核”。后续阅读时，单讲页里的这些段落就是公式前提、作业口径和常见失分点的快速入口。

本轮事实来源优先级为：已校验作业解答与符号导读 → 作业/知识审计文档 → 课件融入审计 → 必要的课程常识转写。原始课件仍只作为本地输入资料，不发布全文。

## 课件来源如何融入

`sources/课件PDF` 只作为本地输入资料，站点不发布原始 PDF。课件里的定义、推导和图像已按“先讲问题、再讲判断、最后做验算”的站点语言融入现有阶段；抽取记录保存在仓库文档 `docs/SOURCE_EXTRACTION_AUDIT.md`，逐页理解和融入位置复核保存在 `docs/COURSE_INTEGRATION_REVIEW.md`。

| 课件主题 | 融入阶段 | 主要补强点 |
|---|---|---|
| 传输线理论、Smith 圆图、阻抗匹配实例 | [01 传播与传输线](01-传播与传输线/README.md)、[02 反射与匹配](02-反射与匹配/README.md) | 长线物理图像、沿线旋转、归一化阻抗/导纳、并联支节匹配流程 |
...
如果目标是从“看过”变成“会用”，建议按十轮推进。每一轮都输出一个明确结果，不要只停留在阅读。
...
| 4 | 公式前提 | 在公式旁标出无耗/有耗、单模/多模、TEM/非 TEM |
...
| 资源 | 说明 |
|------|------|
| [考前复习总览](../guide/exam-review.md) | 考试结构、13 项划重点、公式速查、易混点与电子板习题入口 |
