...
**串讲算例（BLQ p46）**：$23\times10\,\mathrm{mm}$ 铜波导、$10\,\mathrm{GHz}$ $\mathrm{TE}_{10}$ — 先算 $\lambda_{\mathrm c}=4.6\,\mathrm{cm}$，再求 $\lambda_{\mathrm g}$、$v_{\mathrm p}$、$v_{\mathrm g}$、$Z_{\mathrm{TE}}$、$P_{\mathrm{br}}$、$\alpha$ 与传输损耗 $P_L$。步骤与 [Lec13-16 · 第 2 题](../../solutions/03-规则波导与矩形波导/03-Lec13-16/第02题.md) 同型。
...
1. **写尺寸与频率**：$a,b$，$f$ → $\lambda_0=c/f$。
2. **复判单模**：$\lambda_0<2a$？高次模是否仍截止？不满足则停止，不写 $\lambda_{\mathrm g}$。
3. **算 $\lambda_{\mathrm g}$**：
   $$\lambda_{\mathrm g}=\frac{\lambda_0}{\sqrt{1-(\lambda_0/2a)^2}}.$$
   根号里 $(\lambda_0/2a)^2\ge1$ 说明已截止或公式不适用。
4. **算速度**：$v_{\mathrm p}=c/\sqrt{1-(f_{\mathrm c,10}/f)^2}$，$v_{\mathrm g}=c^2/v_{\mathrm p}$（无耗空气主模）。
5. **验算**：$v_{\mathrm p}v_{\mathrm g}\approx c^2$；$\lambda_{\mathrm g}>\lambda_0$；近截止时 $\lambda_{\mathrm g}$ 应变大。
