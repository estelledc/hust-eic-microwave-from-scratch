...
1. 写出本题模的 $\lambda_{\mathrm c}$（或 $k_{\mathrm c}$）。
2. 比较 $\lambda_0$ 与 $\lambda_{\mathrm c}$（等价于比较 $k$ 与 $k_{\mathrm c}$）。
3. 只有 $\lambda_0<\lambda_{\mathrm c}$ 时，才写 $\lambda_{\mathrm g}=\lambda_0/\sqrt{1-(\lambda_0/\lambda_{\mathrm c})^2}$ 或由 $\beta$ 计算。
