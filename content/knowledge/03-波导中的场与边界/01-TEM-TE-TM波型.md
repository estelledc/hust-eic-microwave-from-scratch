...
1. **先定轴向**：题目默认沿 $z$ 导行时，纵向分量就是 $E_z$、$H_z$。
2. **再看结构**：双导体或多导体结构才可能有严格 TEM；空心单导体波导先排除非平凡 TEM。
3. **再看纵向分量**：两个纵向分量都为零是 TEM；只有 $E_z$ 为零是 TE；只有 $H_z$ 为零是 TM。
4. **最后看下标**：$(m,n)$ 说明横截面变化次数，但不能替代 TE/TM 族别。
